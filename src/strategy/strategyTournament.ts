...
import { Coord, GameState, MoveResponse } from "../types/types";
...
export class KweekendStrategy implements Strategy {
  ...
  nextMove(gameState: GameState): MoveResponse {
    ...
    const directionResults: Array<DirectionResult> = Object.values(Direction).map((direction: Direction) => {
      ...
      return this.evaluateDirection(gameState, direction, head, nextCoord)
    });
  ...
  evaluateDirection(gameState: GameState, direction: Direction, head: Coord, nextCoord: Coord): DirectionResult{
    ...
    biggerSnakeScore += isNexNexttToBiggerSnakeHead(nextCoord, gameState, gameState.you.length) ? 50 : 0
    let smallerSnakeScore = isNextToSmallerSnakeHead(nextCoord, gameState, gameState.you.length) ? 50 : 0
    ...
    //Check future in one tick
    //this.evaluateDirection(gameState, direction, head, nextCoord)

    // Collect data to use for sorting
    const score = 
    reachableCells(gameState.board, nextCoord) + 
    (10 - closestFoodDistance) + smallerSnakeScore - 
    biggerSnakeScore - 
    nextToSnakeBodyScore - 
    nextToBorderScore
